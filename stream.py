...
if tag=='Home':     
    ...
    st.markdown("""### Welcome to the Game Analytics application! This tool is designed to manage, visualize, and analyze tennis competition data using the SportRadar API. Explore the world of tennis through  dynamic insights and visualizations,powered by Python, PostgresSQL, and StreamlitAPI:golfer:""")
    st.markdown("### **Work Done By**::crown:_VENKATESHAN_")
    queries = {
        "total_competitors": "SELECT COUNT(*) FROM Competitors;",
        "countries_represented": "SELECT COUNT(DISTINCT country) FROM Competitors;",
        "Top_5_Rank":"""select c.name as competitor_name , c.country , cr.rank,cr.points from competitors c join competitor_rankings cr on c.competitor_id = cr.competitor_id order by cr.rank_id limit 5;  """,
        "High_Competitons_played":"""SELECT c.name AS competitor_name, c.country, cr.competitions_played FROM Competitors c JOIN Competitor_Rankings cr ON c.competitor_id = cr.competitor_id ORDER BY cr.competitions_played DESC LIMIT 10;""",
        "Top_Complex":"SELECT co.complex_name, COUNT(v.venue_id) AS venue_count FROM complexes co JOIN venue v ON co.complex_id = v.complex_id GROUP BY co.complex_name ORDER BY venue_count DESC LIMIT 5;"                    
    }
...
if tag=='Conclusion':
    ...
    Built with **PostgreSQL** for robust data management and **Streamlit** for an interactive user experience, this project highlights the power of modern data visualization tools in making data-driven decisions accessible to everyone.
